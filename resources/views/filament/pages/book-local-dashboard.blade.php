            ...
            <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6" style="display: flex; flex-direction: row; align-items: center; justify-content: space-between; gap: 1.5rem; flex-wrap: wrap;">
                <div class="space-y-1">
                    <h1 class="text-xl font-extrabold text-white tracking-tight" style="font-size: 1.35rem; font-weight: 800; color: #fff;">
                        🏛️ ระบบรับส่งหนังสือราชการ (Local Document Registry)
                    </h1>
                    <p class="text-xs text-gray-400 max-w-2xl" style="font-size: 0.75rem; color: #9ca3af; line-height: 1.5;">
                        ระบบการรับและส่งหนังสือราชการดิจิทัลระหว่างกลุ่มงานในสำนักงานเขตพื้นที่การศึกษาและสถานศึกษาในเครือข่าย
                    </p>
                </div>
                ...
                <div class="px-4 py-1.5 rounded-full text-xs font-bold shadow-lg border transition-all duration-300"
                     ...
                     @elseif($activeTab === 'publish') background-color: rgba(245, 158, 11, 0.1); color: #fbbf24; border-color: rgba(245, 158, 11, 0.2);
                     @elseif($activeTab === 'unreceived_3_days') background-color: rgba(239, 68, 68, 0.1); color: #f87171; border-color: rgba(239, 68, 68, 0.2);
                     @elseif($activeTab === 'older_2_years') background-color: rgba(244, 63, 94, 0.1); color: #fb7185; border-color: rgba(244, 63, 94, 0.2);
                    ...
                    <span class="inline-block w-2 h-2 rounded-full mr-1.5 animate-pulse"
                          ...
                          @elseif($activeTab === 'publish') background-color: #f59e0b;
                          @elseif($activeTab === 'unreceived_3_days') background-color: #ef4444;
                          @elseif($activeTab === 'older_2_years') background-color: #f43f5e;
                          @else background-color: #6366f1; @endif"></span>
                    กำลังดู: @if($activeTab === 'receive') หนังสือรับ
                            @elseif($activeTab === 'send') หนังสือส่ง
                            @elseif($activeTab === 'publish') ส่งหนังสือราชการ
                            @elseif($activeTab === 'unreceived_3_days') หนังสือที่ยังไม่รับเกิน 3 วัน
                            @elseif($activeTab === 'older_2_years') หนังสืออายุเกิน 2 ปี
                            @else คู่มือการใช้งาน @endif
                </div>
            </div>
        </div>

        <!-- 2. HIGHLY STYLISH HORIZONTAL TAB NAVIGATION BAR -->
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 0.75rem;">
            ...
            <button wire:click="setTab('receive')" 
                ...
                <span class="text-[11px] font-bold @if($activeTab === 'receive') text-emerald-400 @else text-gray-400 group-hover:text-white @endif" style="font-size: 0.7rem;">หนังสือรับ</span>
            </button>
            ...
            <button wire:click="setTab('send')" 
                    ...
                    @else background-color: rgba(255, 255, 255, 0.01); border-color: rgba(255, 255, 255, 0.05); @endif">
                <span class="text-xl mb-1 group-hover:scale-110 transition duration-300" style="font-size: 1.2rem;">📤</span>
                <span class="text-[11px] font-bold @if($activeTab === 'send') text-purple-400 @else text-gray-400 group-hover:text-white @endif" style="font-size: 0.7rem;">หนังสือส่ง</span>
            </button>

            <!-- Tab 3: ส่งหนังสือราชการ -->
            <button wire:click="setTab('publish')" 
                    class="group flex flex-col items-center justify-center p-3 rounded-xl border transition-all duration-300 relative overflow-hidden"
                    style="border-radius: 0.75rem; text-align: center; cursor: pointer; outline: none;
                    @if($activeTab === 'publish') background: linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(245, 158, 11, 0.05) 100%); border-color: rgba(245, 158, 11, 0.4); box-shadow: 0 0 15px rgba(245, 158, 11, 0.1);
                    @else background-color: rgba(255, 255, 255, 0.01); border-color: rgba(255, 255, 255, 0.05); @endif">
                <span class="text-xl mb-1 group-hover:scale-110 transition duration-300" style="font-size: 1.2rem;">✉️</span>
                <span class="text-[11px] font-bold @if($activeTab === 'publish') text-amber-400 @else text-gray-400 group-hover:text-white @endif" style="font-size: 0.7rem;">ส่งหนังสือราชการ</span>
            </button>

            <!-- Tab 4: หนังสือที่ยังไม่รับเกิน 3 วัน -->
            <button wire:click="setTab('unreceived_3_days')" 
                    class="group flex flex-col items-center justify-center p-3 rounded-xl border transition-all duration-300 relative overflow-hidden"
                    style="border-radius: 0.75rem; text-align: center; cursor: pointer; outline: none;
                    @if($activeTab === 'unreceived_3_days') background: linear-gradient(135deg, rgba(239, 68, 68, 0.15) 0%, rgba(239, 68, 68, 0.05) 100%); border-color: rgba(239, 68, 68, 0.4); box-shadow: 0 0 15px rgba(239, 68, 68, 0.1);
                    @else background-color: rgba(255, 255, 255, 0.01); border-color: rgba(255, 255, 255, 0.05); @endif">
                <span class="text-xl mb-1 group-hover:scale-110 transition duration-300" style="font-size: 1.2rem;">⏳</span>
                <span class="text-[11px] font-bold @if($activeTab === 'unreceived_3_days') text-red-400 @else text-gray-400 group-hover:text-white @endif" style="font-size: 0.7rem;">ยังไม่รับเกิน 3 วัน</span>
            </button>

            <!-- Tab 5: หนังสืออายุเกิน 2 ปี -->
            <button wire:click="setTab('older_2_years')" 
                    class="group flex flex-col items-center justify-center p-3 rounded-xl border transition-all duration-300 relative overflow-hidden"
                    style="border-radius: 0.75rem; text-align: center; cursor: pointer; outline: none;
                    @if($activeTab === 'older_2_years') background: linear-gradient(135deg, rgba(244, 63, 94, 0.15) 0%, rgba(244, 63, 94, 0.05) 100%); border-color: rgba(244, 63, 94, 0.4); box-shadow: 0 0 15px rgba(244, 63, 94, 0.1);
                    @else background-color: rgba(255, 255, 255, 0.01); border-color: rgba(255, 255, 255, 0.05); @endif">
                <span class="text-xl mb-1 group-hover:scale-110 transition duration-300" style="font-size: 1.2rem;">📁</span>
                <span class="text-[11px] font-bold @if($activeTab === 'older_2_years') text-rose-400 @else text-gray-400 group-hover:text-white @endif" style="font-size: 0.7rem;">อายุเกิน 2 ปี</span>
            </button>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" style="padding: 4rem; text-align: center; color: #9ca3af;">
                                        📭 ไม่พบข้อมูลหนังสือราชการ ตามเงื่อนไขที่กรอง
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            @elseif($activeTab === 'publish')
                ...
                <div class="max-w-2xl mx-auto space-y-6" style="margin: 0 auto;">
                    <div class="relative overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-br from-amber-500/10 via-orange-500/5 to-transparent p-6 shadow-2xl" style="border-radius: 1rem; border: 1px solid rgba(255, 255, 255, 0.05); padding: 1.5rem;">
                        <h2 class="text-lg font-bold text-white mb-1" style="font-size: 1.15rem; font-weight: 800; color: #fff;">✉️ ส่งหนังสือราชการ (Outbound Transmission)</h2>
                        <p class="text-xs text-gray-400" style="font-size: 0.725rem; color: #9ca3af;">จัดเตรียมและนำส่งเอกสารราชการอิเล็กทรอนิกส์ไปยังสถานศึกษาและโรงเรียนปลายทาง</p>
                    </div>
            ...
            @else
                ...
                <div class="space-y-8">
                    ...
                    <div class="relative overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-br from-indigo-500/10 via-blue-500/5 to-transparent p-6 shadow-2xl" style="border-radius: 1rem; border: 1px solid rgba(255, 255, 255, 0.05); padding: 1.5rem;">
                        <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6" style="display: flex; flex-direction: row; align-items: center; justify-content: space-between; gap: 1.5rem; flex-wrap: wrap;">
                            <div class="space-y-1">
                                <h2 class="text-xl font-bold text-white tracking-tight" style="font-size: 1.25rem; font-weight: 800; color: #fff;">📘 คู่มือการใช้งานระบบรับส่งหนังสือราชการ</h2>
                                <p class="text-xs text-gray-400 max-w-xl" style="font-size: 0.725rem; color: #9ca3af; line-height: 1.5;">
                                    เอกสารแนวทางการสืบค้นและส่งต่อเอกสารหนังสือราชการภายในองค์กรและเครือข่ายสถานศึกษา
                                </p>
                            </div>
                            <div class="flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs font-semibold" style="display: inline-flex; align-items: center; gap: 0.5rem; background-color: rgba(99, 102, 241, 0.1); color: #818cf8; border: 1px solid rgba(99, 102, 241, 0.2); border-radius: 9999px; font-size: 0.7rem; padding: 0.25rem 0.75rem;">
                                ...
                                <span>คู่มือระบบทั่วไป v1.0</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem;">
                        ...
                        <div class="bg-white/[0.01] border border-white/5 rounded-xl p-5 shadow-lg" style="border-radius: 0.75rem; border: 1px solid rgba(255,255,255,0.05); padding: 1.25rem;">
                            <div class="flex items-center gap-3 mb-3" style="display: flex; gap: 0.75rem; align-items: center; margin-bottom: 0.75rem;">
                                ...
                                <h3 class="font-bold text-white text-sm" style="font-size: 0.875rem; font-weight: 700;">1. รายการหนังสือรับ (Inbound)</h3>
                            </div>
                            <p class="text-xs text-gray-300 leading-relaxed" style="font-size: 0.7rem; color: #d1d5db; line-height: 1.5;">
                                สำหรับเรียกดูและสืบค้นเอกสารราชการภายนอกที่ส่งเข้ามายังกลุ่มงานต่างๆ โดยสามารถดาวน์โหลดไฟล์แนบ PDF/Word เพื่อนำไปดำเนินการต่อได้ทันที
                            </p>
                        </div>
                        ...
                        <div class="bg-white/[0.01] border border-white/5 rounded-xl p-5 shadow-lg" style="border-radius: 0.75rem; border: 1px solid rgba(255,255,255,0.05); padding: 1.25rem;">
                            <div class="flex items-center gap-3 mb-3" style="display: flex; gap: 0.75rem; align-items: center; margin-bottom: 0.75rem;">
                                ...
                                <h3 class="font-bold text-white text-sm" style="font-size: 0.875rem; font-weight: 700;">2. รายการหนังสือส่ง (Outbound)</h3>
                            </div>
                            <p class="text-xs text-gray-300 leading-relaxed" style="font-size: 0.7rem; color: #d1d5db; line-height: 1.5;">
                                สำหรับเรียกดูและติดตามประวัติเอกสารสำคัญที่ส่งออกไปยังเครือข่ายสถานศึกษาและโรงเรียนในสังกัด เพื่อประสานงาน ติดตามงาน หรือชี้แจงคำสั่งต่างๆ
                            </p>
                        </div>
